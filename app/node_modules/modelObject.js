...
class BaseModel{
    ...
    static async select(data, conn, multi){
    }
...
class SqlModel extends BaseModel{
    ...
    static async count(data, conn, multi, options){
    ...
    //SQL会增加分页和排序两个参数
    static async select(data, conn, multi, options){
        let sql = this.sql('select');
    ...
    static async update(data, conn, multi){
        ...
        let cmd = this.compile(sql, data);   
        //修正多余逗号问题
        cmd.sql = cmd.sql.replace(/set\s+,/, "set ");    
        cmd.conn = conn;
        await db.exec(cmd);
    }
...
class CacheModel extends BaseModel{
    ...
    static async select(data, conn,  multi){  
        ...
        if (data && data.id && (await this.opened())){
            let row = await cache.model.get(this.name(), data.id, multi);
            rows = [row];
        } else {
    ...
    static async update(data, conn, multi){
        ...
        for(let i=0; i<fields.length; i++){
            ...
            sql = i==0? sql : sql+',';
            sql = sql +db.separate(fields[i])+'='+db.param();
            item[fields[i]]=data[fields[i]];
        }
        sql = sql + ' where id='+db.param();
        ...
        if (result.Error) {
            throw result.sqlMessage
        } else if (await this.opened()){
    ...
    static async delete(data, conn, multi){
        if (data.id){
            await db.query({
                conn: conn,
                sql:'delete from '+this.name()+' where id='+db.param(),
                params:[data.id]
            });
            if (this.opened()){
